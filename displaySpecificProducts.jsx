...
function DisplaySpecificProducts({admin, isValid, nameOfTheUser, email}){
    console.log("displaySpecificProducts: boolean admin: "+admin);
    ...
    useEffect(()=>{
        const fetchData=async()=>{
            setLoading(true);
            try{
                const response=await fetch(`https://roastery-website-upgraded-in-progress.onrender.com/api/products/${title}`);
                if(!response.ok){
